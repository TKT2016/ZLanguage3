...
namespace ZLangRT.Descs
{
    ...
    {
        public List<TKTProcArg> Args { get; protected set; }
        ...
        public bool IsNameValue
        {
            get
            {
                return DictArgs.Count > 0 && DictArgs.Count == Args.Count;
            }
        }

        public TKTProcBracket(IEnumerable<TKTProcArg> listArgs)
        {
            Args = new List<TKTProcArg>(listArgs);
            ...
            CanAjust = checkCanAjust(HasArgName, Args);
        }
        ...
        {
            if (this.Args.Count != anthor.Args.Count) return false;
            for (int i = 0; i < Args.Count; i++)
            {
                if (!this.Args[i].Eq(anthor.Args[i]))
                {
                    return false;
                }
            }
