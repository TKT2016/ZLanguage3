...
using ZCompileNLP.Segment.FinalSeg;
...
namespace ZCompileNLP.Segment
{
    public class JiebaSegmenter
    {
        ...
        public IEnumerable<SegToken> Tokenize(string text, TokenizerMode mode = TokenizerMode.Default, bool hmm = true)
        {
            var result = new List<SegToken>();
            ...
            if (mode == TokenizerMode.Default)
            {
                ...
                {
                    var width = w.Length;
                    result.Add(new SegToken(w, start, start + width));
                    start += width;
                }
            }
            else
                ...
                foreach (var w in Cut(text, hmm: hmm))
                {
                    ...
                    if (width > 2)
                    {
                        for (var i = 0; i < width - 1; i++)
                        {
                            ...
                            if (WordDict.ContainsWord(gram2))
                            {
                                result.Add(new SegToken(gram2, start + i, start + i + 2));
                            }
                        }
                    }
                    if (width > 3)
                    {
                        for (var i = 0; i < width - 2; i++)
                        {
                            ...
                            if (WordDict.ContainsWord(gram3))
                            {
                                result.Add(new SegToken(gram3, start + i, start + i + 3));
                            }
                        }
                    }
